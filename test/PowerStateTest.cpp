...
static const QString kIdle = QStringLiteral("Нажмите для подключения");
...
/** Оболочка на экране и её кнопка, подпись под кнопкой и список серверов. */
struct Shell {
    GreenRhythm::MainShell w;
    QPushButton *power = nullptr;
    QLabel *hint = nullptr;
    QComboBox *pick = nullptr;
    int toggled = 0;
    int chosen = -100;
    ...
    Shell() {
        ...
        pick = w.findChild<QComboBox *>(QStringLiteral("grServerPick"));
    ...
    bool ready() const { return power != nullptr && hint != nullptr && pick != nullptr; }
...
int main(int argc, char **argv) {
    ...
    {
        Shell s;
        is(QStringLiteral("кнопка, подпись и список серверов найдены"), s.ready());
...

    {
        ...
        s.w.setState(State::Failed, kReason);
        QMetaObject::invokeMethod(s.pick, "activated", Qt::DirectConnection, Q_ARG(int, 1));
