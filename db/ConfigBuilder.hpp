...
namespace NekoGui {
    ...
    public:
        ...
        // Правила, которые обязаны стоять ПЕРЕД цепочкой из пресета. Туда попадает
        // только то, что человек назвал поимённо: его выбор конкретнее любого
        // общего запрета и потому должен решать раньше. Список из пресета блокирует
        // весь udp/443, и названная игра, дописанная в конец, не спасалась бы им
        // вовсе — на этом сгорел Squad.
        // Перехват разрешения имён. Отдельно и раньше всех: обогнавший его
        // пользовательский список оставляет человека без разрешения имён вовсе.
        QJsonArray routingRulesDns;

        QJsonArray routingRulesFirst;
        QJsonArray inbounds;
