...
namespace NekoGui_fmt {
    ...
    ExternalBuildResult RelayBean::BuildExternal(int mapping_port, int socks_port, int external_stat) {
        ...
        if (result.program.isEmpty()) {
            // ТЕКСТ НЕ ДОЛЖЕН ОТПРАВЛЯТЬ ЗА НЕСУЩЕСТВУЮЩИМ — и дважды подряд
            // отправлял. Сперва здесь стояло «восстановите файл из архива»:
            // ложь для сборки, в которую компонент не клали вовсе. Потом —
            // «переустановите приложение», и это оказалось ложью ровно так же:
            // компонент не входит в приложение и входить не будет, он приезжает
            // по подписке. Переустановка не изменила бы ничего, кроме
            // потраченного вечера.
            //
            // Правильное действие ровно одно, и оно называется целиком, вместе
            // с путём до кнопки: человек, дошедший до этой строки, уже нажал
            // «Подключиться» и ждёт, что ему скажут, куда идти.
            result.error = QObject::tr(
                "Компонент резервного подключения не установлен. Откройте «Зелёный Ритм» → "
                "«Резервное подключение» и нажмите «Загрузить компонент».");
            return result;
        }
