...
namespace NekoGui_fmt {
    ...
    ExternalBuildResult RelayBean::BuildExternal(int mapping_port, int socks_port, int external_stat) {
        ...
        if (result.program.isEmpty()) {
            // ТЕКСТ НЕ ДОЛЖЕН ОТПРАВЛЯТЬ ЗА НЕСУЩЕСТВУЮЩИМ. Здесь стояло
            // «восстановите файл из архива или добавьте в исключения
            // антивируса» — и это ложь для сборки, в которую компонент не
            // клали вовсе: человек пошёл бы перекачивать архив и копаться в
            // антивирусе, а искать там нечего.
            //
            // Причин ровно две, и они разные по действию, поэтому и сказаны
            // раздельно. Пункт меню при отсутствии компонента вообще скрыт
            // (ui/mainwindow.cpp), так что сюда попадают только те, у кого он
            // БЫЛ и пропал.
            result.error = QObject::tr(
                "Компонент резервного подключения не запускается. Если он есть рядом с "
                "программой — добавьте её в исключения антивируса; если файла нет — "
