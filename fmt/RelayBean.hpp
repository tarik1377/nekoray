...
namespace NekoGui_fmt {
    ...
    public:
        ...
        /**
         * Звонки и игры — напрямую, мимо канала.
         *
         * ЗАЧЕМ ЭТО ЕСТЬ. Через транспорт резерва разговор невозможен: круг
         * «сказал — услышал» там измеряется секундами, а разговор ломается уже
         * на четверти. Датаграмму можно упаковать в поток; сделать по такому
         * кругу разговор — нельзя. Единственный способ, которым звонок может
         * состояться при работающем резерве, — выпустить датаграмму мимо канала.
         *
         * ВЫКЛЮЧЕНО ПО УМОЛЧАНИЮ, в отличие от соседнего обхода. Разница не в
         * осторожности, а в том, ЧТО именно уходит наружу: российские сайты и
         * так открываются с этого адреса, а собеседник по звонку узнаёт
         * настоящий адрес человека, которого до сих пор не знал.
         *
         * ЧЕГО НЕ ДАЁТ. Если голос режут в самой сети человека, прямая дорога
         * идёт по той же сети и упрётся в тот же запрет.
         */
        bool udp_direct = false;
        ...
        RelayBean() : AbstractBean(0) {
            ...
            _add(new configItem("udp_direct", &udp_direct, itemType::boolean));
