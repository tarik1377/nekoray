...
namespace GreenRhythm {
    ...
    QWidget *MainShell::buildSettingsPage() {
        ...
        // Windows: задача планировщика с наивысшими правами (sys/AutoRunTask.hpp) —
        // разрешение спрашивают один раз, дальше туннель при входе без вопросов.
        addRow(tr("Запуск вместе с системой"), tr("При входе в систему программа запустится сама и будет ждать в трее. Windows один раз спросит разрешение, чтобы туннель поднимался без вопросов."), "gr-power", autostartToggle);
#else
        // Автозапуск Linux передаёт -tray (sys/AutoRun.cpp): окно не откроется.
