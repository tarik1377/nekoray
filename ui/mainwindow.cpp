...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent), ui(new Ui::MainWindow) {
    ...
    // привязан весь код выше.
    {
        ...
        // ПРЕЖНИЙ ЦЕНТРАЛЬНЫЙ ВИДЖЕТ ЗАБИРАЕТСЯ, А НЕ ОТДАЁТСЯ НА УДАЛЕНИЕ.
        //
        // setCentralWidget удаляет того, кто стоял до него. Вместе с ним умирало
        // всё, что лежало внутри и не было перенесено: строка состояния, поле
        // поиска, разделитель, обе галки, ряд кнопок. adopt() выше забирает
        // только таблицу серверов и вкладки журнала.
        //
        // Дальше код продолжал писать в покойников: refresh_status — в метки по
        // таймеру раз в две секунды, on_commitDataRequest — в разделитель при
        // КАЖДОМ штатном выходе, обработчик Esc — в поле поиска на любое
        // нажатие. Проверки на nullptr тут бессильны: указатель после удаления
        // не нулевой, он мусорный.
        //
        // Ценой была сборка macOS: приложение падало на второй секунде запуска,
        // SIGSEGV в QLabel::setText из refresh_status, адрес 0x389. Под Windows
        // тот же промах молчал — освобождённый блок оставался читаемым.
        //
        // takeCentralWidget передаёт владение нам, и Qt его не трогает. Родителя
        // ставим новому — иначе виджет протечёт. Прятать обязательно: без
        // родительской раскладки он всплыл бы отдельным окном.
        //
        // Это заплата, и честный ремонт другой: перенести строку состояния и
        // поиск в оболочку так же, как adopt переносит таблицу, либо убрать код,
        // который в них пишет. Пока не сделано — legacy обязан жить.
        // Сторож: test/WidgetLifetimeTest.cpp.
        QWidget *legacy = takeCentralWidget();
