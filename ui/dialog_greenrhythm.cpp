...
// Обе функции есть и на macOS — там их пункты меню намеренно оставлены видимыми.
// Пряча их в панели, мы делали «наши функции в одном месте» беднее того подменю,
// из которого панель и выросла. На Linux строки по-прежнему не собираются, поэтому
// сигнал не испускается и trigger() у скрытого действия не случается.
#if defined(Q_OS_WIN) || defined(Q_OS_MACOS)
    page->addWidget(actionRow(
        this, tr("Починить сеть"),
#ifdef Q_OS_MACOS
        // На маке действие шире и дороже: оно ещё и показывает чужие туннели, а
        // для сброса кэша имён система спрашивает пароль. Умолчать про пароль
        // значит подставить человека под неожиданное окно с требованием пароля.
        tr("Вернёт системные настройки, сбросит кэш имён и покажет сторонние туннели. "
           "Система спросит пароль администратора."),
#else
    ...
    page->addWidget(actionRow(
        ...
        tr("Покажет чужие туннели и их маршруты. Ничего не выключается."),
