...
# ФАЙЛОВАЯ СИСТЕМА ВНУТРИ ОБРАЗА ЗАКРЕПЛЕНА, И ЭТО НЕ ПЕРЕСТРАХОВКА.
#
# Без -fs её выбирает та система, на которой образ собран, а собираем мы на
# macOS 15. Мак постарше такой образ не откроет вовсе — и скажет об этом ровно
# так, как увидел тестер 1.8.0: «Не удалось открыть образ диска. Ошибка
# ввода/вывода». Файл при этом целый: сумма сходится, хвост koly на месте, и
# сборка сама его смонтировала строчкой ниже — просто монтировала на той же
# новой системе, где он и родился.
#
# HFS+ читает любая macOS, а образ у нас только для чтения, так что терять на
# этом нечего. UDZO (zlib) выбран по той же причине и остаётся: ULFO и ULMO
# требуют систем поновее.
